...
This project is built against Ubuntu. Please consider checking your Linux distribution and changing it accordingly to distribution needs.
...
### Create S3 Bucket
Configure naming rules to only include lowercase letters and remove uppercase as it is a requirement in naming S3 buckets.
...
When attempting to run `terraform login` it will launch bash a with view to generate a token. However, it does not work as expected in Gitpod VsCode in the browser.
